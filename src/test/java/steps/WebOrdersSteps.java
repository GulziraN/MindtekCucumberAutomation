...
public class WebOrdersSteps {
    ...
    @When("User logs in with username and {string} and password {string}")
    public void user_logs_in_with_username_and_and_password(String username, String password) {
    ...
    public void user_is_is_successfully_logged_in_and_lands_on_the_homepage() {
    ...
    public void user_creates_order_with_data(DataTable dataTable) {
        ...
        for (int i=0; i<data.size(); i++){
            ...
            webOrdersOrderPage.inputStreet.sendKeys(data.get(i).get("CITY").toString());
    ...
    public void user_validates_created_order_is_in_the_list_of_all_orders() {
        ...
        Assert.assertEquals(data.get(1).get("CUSTOMER NAME").toString(), webOrdersHomePage.firstRowOrder.get(1).getText());
